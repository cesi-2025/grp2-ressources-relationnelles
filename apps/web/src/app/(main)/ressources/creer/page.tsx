...
export default function CreateResourcePage() {
  ...
  useEffect(() => {
    ...
    getResources({ per_page: "60" }).then((res) => {
      ...
      setRelationTypes(
        uniqueById(list.map((r) => r.relation_type).filter(Boolean) as Meta[]),
      );
      setResourceTypes(
        uniqueById(list.map((r) => r.resource_type).filter(Boolean) as Meta[]),
      );
    }).catch(() => {});
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!categoryId || !relationTypeId || !resourceTypeId) {
      setGlobalError("Veuillez renseigner tous les champs de classification.");
      return;
    }
  ...
  return (
    <div className="bg-gray-50 min-h-screen py-12">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Card>
          <h1 className="text-3xl font-bold text-primary mb-3">Créer une ressource</h1>
          <p className="text-sm text-gray-600 mb-6">
            Votre ressource sera soumise à modération avant d&apos;être publiée.
          </p>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="create-content">
                Contenu
              </label>
              <textarea
                id="create-content"
            ...
            <label className="flex items-center gap-2 text-sm text-gray-700">
              ...
              />
              Ressource publique (visible par tous les citoyens une fois validée)
            </label>

            <div className="pt-2 flex gap-2">
              <Button variant="primary" type="submit" disabled={submitting}>
                {submitting ? "Envoi..." : "Soumettre la ressource"}
              </Button>
